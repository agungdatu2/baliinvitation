...
// `section` di sini adalah key REVERIE_SECTION_KEYS yang bersangkutan (null =
// tidak bisa di-hide) — dipakai buat filter link kalau section-nya disembunyikan
// client dari admin dashboard.
const LINK_HREFS = [
...
// Copy dari NavMenu Muse/Reverie (glassmorphism: blur + transparan, teks putih)
// — beda cuma di ARAH SLIDE: panel ini turun dari ATAS (translate-y), bukan
// masuk dari kanan (translate-x) seperti tema lain. Tipografi link pakai voice
// Nocturne sendiri (font-nocturne-display).
        ...
        <nav className="flex flex-col gap-4 mt-16">
          {visibleLinks.map((l) => (
            <a
              ...
              href={l.href}
              onClick={() => setOpen(false)}
              className="font-nocturne-display text-2xl md:text-3xl text-white hover:text-white/70 transition-colors"
            >
              {t[l.key]}
            </a>
          ))}
