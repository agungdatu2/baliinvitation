...
- Struktur baru di `ReverieTemplate.tsx`: `<div className="md:flex">` membungkus kolom kiri (`md:w-[30%]`, isi: Hero + `.groove-page-blur` semua section + footer, scrollable) dan kolom kanan (`md:w-[70%]`, `hidden` di mobile, sticky). Rasio 70/30 mengikuti urutan yang diminta user (sticky:scrollable = 70:30) — sempat kebalik di percobaan pertama (scrollable dikira 70%), sudah diperbaiki. Di dalam kolom kanan ada wrapper `md:sticky md:top-0 md:h-screen` berisi satu foto besar (`stickyPhoto` — foto pertama dari galeri yang bukan video, fallback Picsum `reverie-sticky`).
