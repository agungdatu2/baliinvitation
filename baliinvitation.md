...
- Struktur di `ReverieTemplate.tsx`: `<div className="md:flex">` membungkus kolom **kiri** (`md:w-[70%]`, `hidden` di mobile, sticky, isi satu foto besar `stickyPhoto` — foto pertama dari galeri yang bukan video, fallback Picsum `reverie-sticky`) dan kolom **kanan** (`md:w-[30%]`, isi: Hero + `.groove-page-blur` semua section + footer, scrollable). Rasio 70/30 = sticky:scrollable. Urutan kiri/kanan sempat foto-di-kanan lalu ditukar ke foto-di-kiri atas permintaan user — kalau mau tukar lagi, tinggal tukar urutan dua `<div>` kolom di JSX (width class-nya tidak perlu diubah, cuma urutannya).
