...
- **Aturan standar (bukan fitur baru, cuma dicatat)**: semua foto/video background di panel sticky & kolom scrollable WAJIB `object-cover` + center (tanpa override `object-position`). Sudah diaudit — semua yang ada sekarang (`stickyPhoto`, background gate di `SplashGate`, foto `LoadingScreen`, `FixedVideoBackground`) sudah patuh, tidak ada yang perlu diubah. Berlaku otomatis untuk background baru ke depannya tanpa perlu diminta ulang.
- **Tombol "Let's Open" (gate) diperkecil**: dari `w-full` jadi `w-4/5 mx-auto` (80% lebar, tetap center).
- **Hero (`HeroGreeting.tsx`) direstyle** mengikuti referensi foto (emblem bunga kecil di atas teks ajakan, nama besar, tanggal lengkap) — **indikator SCROLL di bawah sengaja TIDAK diubah** sesuai permintaan eksplisit user ("scroll animation tetap"):
  - Tambah ikon `Flower2` (lucide-react) kecil di atas eyebrow text.
  - Eyebrow text ganti dari `t.theWeddingOf` ("THE WEDDING OF", dipakai bersama di gate) jadi key baru `t.heroInviteLabel` ("Kami mengundang Anda untuk merayakan" / "We invite you to celebrate") — key baru khusus Hero supaya gate tidak ikut berubah.
  - Format tanggal Hero diubah dari singkat (`15/09/2026`) jadi lengkap dengan hari (`Selasa, 15 September 2026`), konsisten dengan format tanggal yang sudah dipakai di gate.
  - Paragraf greeting ("Om Swastyastu...") sengaja **dipertahankan** (tidak ada di referensi foto, tapi itu konten asli yang bisa di-custom admin — tidak dihapus tanpa diminta eksplisit).
- Font nama pengantin di Hero di-fix jadi `text-[36px]` (ganti dari responsive `text-5xl md:text-7xl lg:text-[5.5rem]`, sekarang satu ukuran tetap di semua breakpoint).

### 9. Section "Doa" baru + scroll-snap — SELESAI
- Komponen baru `src/components/templates/reverie/PrayerSection.tsx`, dirender di `ReverieTemplate.tsx` tepat di bawah Hero (di luar `.groove-page-blur`, karena section ini punya foto background sendiri yang opaque — tidak butuh video di-blur di belakangnya).
- **Reuse field yang sudah ada** (bukan nambah kolom DB baru): `data.coverImage` = foto background section (fallback foto Unsplash pasangan jalan di pantai kalau kosong), `data.quote` = teks doa/kutipan (fallback `t.defaultPrayerQuote`, dictionary key baru id/en). Label "Doa"/"Prayer" juga dictionary key baru (`t.prayerLabel`).
- **z-index**: section `relative z-10` (pasti di atas `FixedVideoBackground` yang `fixed -z-10` di level `<main>`), foto & gradient overlay di dalam section pakai `-z-10` tapi itu lokal ke section ini sendiri (bukan level halaman) karena section-nya `relative` (stacking context baru) — tidak konflik dengan video.
- **Scroll-snap**: class `.reverie-snap-scroll` baru di `globals.css` (`scroll-snap-type: y mandatory`, cuma didefinisikan untuk `html.reverie-snap-scroll`, TIDAK aktif untuk halaman/tema lain) — di-toggle ke `document.documentElement` lewat `useEffect` di `ReverieTemplate` HANYA saat `opened` true (gate/loading tidak perlu snap). Section yang mau nge-snap ditandai `snap-start` (Tailwind, = `scroll-snap-align: start`): Hero, `PrayerSection`, dan semua section yang lewat `Reveal.tsx` (CoupleProfile, LoveStory, EventDetails, LiveStreaming, DressCode, RSVP, Gallery, WeddingGift — `Reveal` sekarang otomatis nambah `snap-start` ke wrapper-nya, jadi section baru yang lewat `Reveal` ke depannya otomatis ikut snap tanpa perlu diset manual). `ClosingFooter` (dirender di luar `Reveal`) sengaja tidak ditandai snap (section penutup, tidak masalah kalau tidak snap-align).
- Sudah dites: scroll-snap kerja (posisi scroll "ketarik" ke titik snap terdekat baik di mobile 375px maupun desktop 1280px), panel sticky tetap `top:0` normal meski scroll-snap aktif, video FixedVideoBackground tetap keliatan blur normal di section-section sesudah Doa (tidak ada konflik layer), tidak ada error console.
- **Gap ditemukan & diperbaiki**: user tanya apakah field-nya sudah ada di admin form — ternyata `coverImage` (foto) sudah ada di form, tapi `quote` (teks doa) **belum**, padahal sudah ada di schema/type/API (cuma tidak pernah dipasang inputnya di `InvitationForm.tsx`). Ditambahkan: `<Field>` textarea "Kutipan / Doa untuk Section Doa" (khusus muncul kalau `templateKey === "reverie"`, sama seperti `reverieGateImage`), plus `quote` ditambahkan ke `defaultValues` form & `initialValues` di halaman edit (`src/app/admin/invitations/[id]/edit/page.tsx`) yang sebelumnya juga kelewatan. Label "Doa"/"Prayer" & fallback quote sudah otomatis ikut `data.language` (dictionary `lume.ts`), tidak perlu field tambahan untuk itu.
