...
- Struktur di `ReverieTemplate.tsx`: `<div className="md:flex">` membungkus kolom **kiri** (`md:w-[35%]`, `hidden` di mobile, sticky, isi satu foto besar `stickyPhoto` — foto pertama dari galeri yang bukan video, fallback Picsum `reverie-sticky`) dan kolom **kanan** (`md:w-[65%]`, isi: Hero + `.groove-page-blur` semua section + footer, scrollable). Rasio sticky:scrollable sudah diubah beberapa kali sesuai arahan (70:30 → 35:65) — kalau mau ubah lagi tinggal ganti dua angka `md:w-[N%]` ini (harus saling melengkapi 100%). Urutan kiri/kanan: foto sticky di kiri, konten scrollable di kanan (posisi sempat ditukar juga atas permintaan user — tinggal tukar urutan dua `<div>` kolom di JSX kalau mau tukar lagi).
...
- **Konten kolom scrollable disamakan dengan tampilan mobile** — karena kolom kanan cuma 65% lebar viewport (bukan full-width), breakpoint Tailwind `md:`/`lg:` (berbasis lebar VIEWPORT, bukan lebar container/kolom) tetap memicu layout grid 2-3 kolom & ukuran font desktop yang menyempit/kepenuhan. Semua class `md:`/`lg:` yang mengubah layout & ukuran font di komponen section Reverie **dihapus** (bukan pakai container query — sengaja disederhanakan, tetap konsisten dengan tampilan mobile yang sudah ada, bukan bikin breakpoint baru): `CoupleProfile.tsx` (flex-row→selalu flex-col, prop `reverse` dihapus total karena sudah tidak dipakai), `LoveStory.tsx`, `EventDetails.tsx`, `RSVPForm.tsx`, `WeddingGift.tsx` (grid 2 kolom → selalu 1 kolom/stack), `Gallery.tsx` (masonry 3 kolom → tetap 2 kolom), `DressCode.tsx`, `ClosingFooter.tsx`, `HeroGreeting.tsx` (ukuran font md:/lg: dihapus, pakai skala mobile terus). Kalau nanti mau section tertentu balik ke layout desktop lagi (misal EventDetails), tinggal tambahkan lagi `md:grid-cols-2` dkk ke class yang relevan.
